...
------------------------------------
The same Markdown in ``docs/`` reaches two places:
...
The application used to serve a third — an unauthenticated ``/docs/public`` —
and that route was removed; ``tests/test_public_docs.py`` asserts it stays a
404. This registry is kept in ``app/services/`` all the same, because the
in-product links are built from it (:func:`site_url`) and because the day a
document is served from the application again, the redaction table has to be
the same one.

The PUBLISHED surface has to agree with the application on what may be
published and on what is rewritten before it is. While that list and the redaction table lived inside
...
PUBLIC_DOCS: list[tuple[str, str, str, str, str]] = [
    ...
    ("docker-compose.md", "docker-compose", "Docker Compose — operator manual", "\U0001f9f0",
     "Running the container stack: guided and manual install, every service "
     "with its user, ports, volumes and permissions, the four renounced "
     "capabilities and the procedures that replace them, configuration, TLS, "
     "primary/standby failover, update, backup and troubleshooting."),
