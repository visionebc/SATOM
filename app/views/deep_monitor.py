...
SPEC = mp.PageSpec(
    ...
    footnote=(
        ...
        "cadence, not instantly, and it is not offered on FortiAuthenticator at "
        "all &mdash; that product exposes no interface resource to watch. "
        "Processor load and memory usage read "
        "<code>get system performance</code> over the read-only CLI on FortiWeb "
        "and FortiADC, and <code>/api/v1/systeminfo/</code> over REST on "
        "FortiAuthenticator, which answers both CLI commands with the literal "
        "string <code>No such command.</code> &mdash; a successful round trip "
        "carrying no reading. <code>proxyd</code> reads <code>diagnose system top</code> "
