...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --out)           OUT="$2"; shift 2 ;;
        # [SATOM-NGINX-DEFAULT] Claim the default server on both listeners.
        # Without an explicit owner nginx awards it to the FIRST server block
        # for that port in parse order, which between files is ALPHABETICAL --
        # so a stray `default.conf` beats `satom.conf` and answers every
        # request that arrives by an unmatched name. Opt-in, because on a host
        # that already serves another site claiming it makes nginx refuse the
        # whole configuration with "duplicate default server".
        --default-server) DEFAULT_SERVER=" default_server"; shift ;;
