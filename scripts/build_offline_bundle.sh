#!/usr/bin/env bash
#
# Build an OFFLINE install bundle for SATOM (web).
#
...
#   * dist/satom-offline-<date>.tar.gz — app source + wheelhouse
#
# On the target:
#   tar xzf satom-offline-<date>.tar.gz -C /opt
#   cd /opt/satom && ./scripts/install.sh --offline
set -euo pipefail

cd "$(dirname "$0")/.."          # app root
ROOT="$(pwd)"
DATE="${1:-$(date +%Y%m%d)}"     # allow caller to pass a fixed stamp
OUT="dist/satom-offline-$DATE.tar.gz"
...
echo "  tar xzf $(basename "$OUT") -C /opt && cd /opt/satom && ./scripts/install.sh --offline"
