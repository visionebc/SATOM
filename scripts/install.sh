#!/usr/bin/env bash
#
# OFortMAuT (web) — installer.  Provisions a fresh host OR re-runs safely
...
if [ ! -f .env ]; then
  ...
  cat > .env <<ENVEOF
# OFortMAuT environment — generated by install.sh on first install.
...
mkdir -p /var/log/ofortmaut
cat > "/etc/systemd/system/$SERVICE.service" <<UNITEOF
[Unit]
Description=OFortMAuT Web App
...
ExecStart=$APP_DIR/venv/bin/gunicorn --workers 4 --bind 0.0.0.0:$PORT --timeout 120 --access-logfile /var/log/ofortmaut/access.log --error-logfile /var/log/ofortmaut/error.log wsgi:app
Restart=always
RestartSec=5
...
if timeout 30 bash -c "until curl -sfo /dev/null http://127.0.0.1:$PORT/auth/login; do sleep 1; done"; then
  log "✓ OFortMAuT is UP on port $PORT."
  echo
