#!/usr/bin/env bash
#
# Fortinet Manager (web) — installer.  Provisions a fresh host OR re-runs safely
...
APP_DIR="${APP_DIR:-/opt/fortinet-manager}"
DB_NAME="${DB_NAME:-fortinet_mgr}"
DB_USER="${DB_USER:-fortinet}"
DB_HOST="${DB_HOST:-127.0.0.1}"
DB_PORT="${DB_PORT:-5432}"
SERVICE="${SERVICE:-fortinet-manager}"
PORT="${PORT:-8000}"
OFFLINE=0
SYSTEM_DEPS=1
...
if [ ! -f .env ]; then
  ...
  cat > .env <<ENVEOF
# Fortinet Manager environment — generated by install.sh on first install.
...
mkdir -p /var/log/fortinet-manager
cat > "/etc/systemd/system/$SERVICE.service" <<UNITEOF
[Unit]
Description=Fortinet Manager Web App
...
ExecStart=$APP_DIR/venv/bin/gunicorn --workers 4 --bind 0.0.0.0:$PORT --timeout 120 --access-logfile /var/log/fortinet-manager/access.log --error-logfile /var/log/fortinet-manager/error.log wsgi:app
Restart=always
RestartSec=5
...
if timeout 30 bash -c "until curl -sfo /dev/null http://127.0.0.1:$PORT/auth/login; do sleep 1; done"; then
  log "✓ Fortinet Manager is UP on port $PORT."
  echo
