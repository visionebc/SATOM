...
Route B: the admin runs ARBITRARY Python, but inside a bubblewrap sandbox that
  * cannot see /opt/fortinet-manager/.env or any secret file,
...
def test_cannot_read_env_secret_file():
    r = run("import os; print('exists=', os.path.exists('/opt/fortinet-manager/.env'))")
...
def test_cannot_write_outside_sandbox():
    src = (
        "try:\n"
        "    open('/opt/fortinet-manager/pwned.txt', 'w').write('x')\n"
        "    print('WROTE')\n"
        "except Exception as e:\n"
